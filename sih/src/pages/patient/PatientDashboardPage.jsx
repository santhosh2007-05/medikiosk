...
import {
  ...
  Clock, Stethoscope, Eye, Activity, ArrowRight, Globe, Check, Mic, Sparkles
} from 'lucide-react';
...
export const PatientDashboardPage = ({ onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-stone-100 text-stone-900 flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-64 bg-stone-900 text-white p-5 flex flex-col justify-between shrink-0 shadow-xl sticky top-0 h-screen overflow-y-auto">
        <div className="space-y-6">
          ...
          <button
            ...
            className="w-full p-2.5 rounded-xl bg-stone-800 border border-stone-700 hover:border-emerald-500 text-xs font-bold text-emerald-400 flex items-center justify-between transition"
          >
            <span className="flex items-center gap-2">
              <Globe className="w-4 h-4 text-emerald-400" />
              <span>{lang === 'ta-IN' ? 'தமிழ் (Tamil)' : lang === 'hi-IN' ? 'हिन्दी (Hindi)' : lang === 'te-IN' ? 'తెలుగు (Telugu)' : 'English'}</span>
            </span>
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-900/60 text-emerald-300 font-semibold">Change</span>
          </button>
          ...
          <nav className="space-y-1 text-xs font-semibold">
            <button
              ...
              className={`w-full p-3 rounded-xl flex items-center gap-3 transition ${
                activeTab === 'overview' ? 'bg-emerald-600 text-white shadow-md font-bold' : 'text-stone-300 hover:bg-stone-800'
              }`}
            >
            ...
            <button
              ...
              className={`w-full p-3 rounded-xl flex items-center gap-3 transition ${
                activeTab === 'intake' ? 'bg-emerald-600 text-white shadow-md font-bold' : 'text-emerald-400 hover:bg-stone-800 bg-stone-950/60 border border-emerald-500/30'
              }`}
            >
            ...
            <button
              ...
              className={`w-full p-3 rounded-xl flex items-center gap-3 transition ${
                activeTab === 'visits' ? 'bg-emerald-600 text-white shadow-md font-bold' : 'text-stone-300 hover:bg-stone-800'
              }`}
            >
            ...
            <button
              ...
              className={`w-full p-3 rounded-xl flex items-center gap-3 transition ${
                activeTab === 'documents' ? 'bg-emerald-600 text-white shadow-md font-bold' : 'text-stone-300 hover:bg-stone-800'
              }`}
            >
            ...
            <button
              ...
              className={`w-full p-3 rounded-xl flex items-center gap-3 transition ${
                activeTab === 'abha' ? 'bg-emerald-600 text-white shadow-md font-bold' : 'text-stone-300 hover:bg-stone-800'
              }`}
            >
            ...
            <button
              ...
              className={`w-full p-3 rounded-xl flex items-center gap-3 transition ${
                activeTab === 'book' ? 'bg-emerald-600 text-white shadow-md font-bold' : 'text-stone-300 hover:bg-stone-800'
              }`}
            >
        ...
        <div className="pt-4 border-t border-stone-800 space-y-3">
          <div className="rounded-xl overflow-hidden border border-stone-800 relative h-24">
            <img src={MEDICAL_IMAGES.tnHospital} alt="Government Hospital" className="w-full h-full object-cover opacity-80" />
            <div className="absolute inset-0 bg-gradient-to-t from-stone-950 via-stone-950/40 to-transparent p-2 flex flex-col justify-end">
          ...
          <button
            onClick={onLogout}
            className="w-full py-2 bg-stone-800 hover:bg-rose-900/40 text-stone-300 hover:text-rose-400 rounded-xl text-xs font-semibold flex items-center justify-center gap-2 transition"
          >
      ...
      {/* MAIN CONTENT AREA */}
      <main className="flex-1 p-4 sm:p-6 md:p-8 w-full space-y-6 overflow-y-auto">
        {/* OVERVIEW TAB */}
        {activeTab === 'overview' && (
          <div className="space-y-6">
            {/* Interactive Hero Banner */}
            <div className="relative rounded-3xl overflow-hidden bg-stone-900 text-white shadow-lg border border-stone-200">
              <img src={MEDICAL_IMAGES.patientBanner} alt="Patient Banner" className="w-full h-48 sm:h-56 object-cover opacity-45" />
              <div className="absolute inset-0 bg-gradient-to-r from-stone-950 via-stone-950/80 to-transparent p-6 sm:p-8 flex flex-col justify-between">
                <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => setLangModalOpen(true)}
                    className="px-4 py-3 bg-stone-800/80 hover:bg-stone-700 text-stone-200 border border-stone-600 font-bold rounded-xl text-xs flex items-center gap-2 transition"
                  >
                    <Globe className="w-4 h-4 text-emerald-400" /> <span>மொழி / Language</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-white border border-stone-200 p-5 rounded-2xl shadow-xs space-y-2">
                ...
                <div className="text-3xl font-black text-stone-900">{visitHistory.length} Visits</div>
                <div className="text-[11px] text-stone-500">{visitHistory.length === 0 ? "Fresh Account (No Past Visits)" : "Last visit active"}</div>
              </div>
              ...
              <div className="bg-white border border-stone-200 p-5 rounded-2xl shadow-xs space-y-2">
                ...
                <div className="text-lg font-bold text-stone-900 font-mono">{abhaNumber}</div>
                <div className="text-[11px] text-emerald-600 font-semibold">Linked with Aadhaar ({aadhaarNumber.slice(-4)})</div>
              </div>
              ...
              <div className="bg-white border border-stone-200 p-5 rounded-2xl shadow-xs space-y-2">
                ...
                <div className="text-[11px] text-stone-500">Tamil Nadu Government OPD</div>
              </div>
            </div>
            ...
            <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-xs space-y-4">
              <div className="flex items-center justify-between border-b border-stone-100 pb-3">
                <h3 className="font-bold text-sm text-stone-900 flex items-center gap-2">
                  <Clock className="w-4 h-4 text-emerald-600" /> {getTranslation("myVisitsReports", lang)}
                </h3>
                {visitHistory.length > 0 && (
                  <button onClick={() => setActiveTab('visits')} className="text-xs text-emerald-600 hover:underline font-semibold">
              ...
              {visitHistory.length === 0 ? (
                <div className="p-6 bg-stone-50 border border-stone-200 rounded-2xl text-center space-y-4">
                  <div className="max-w-md mx-auto h-40 rounded-2xl overflow-hidden shadow-sm relative border border-stone-200">
                    <img src={MEDICAL_IMAGES.consultation} alt="Doctor Consultation" className="w-full h-full object-cover" />
                    <div className="absolute inset-0 bg-stone-950/30 flex items-center justify-center">
                      <span className="text-white text-xs font-bold bg-stone-900/80 px-3 py-1.5 rounded-xl border border-stone-700">
                        ✨ MediKiosk • {getTranslation("welcomeBack", lang)}
                      </span>
                    </div>
                  </div>
                  <div className="space-y-1">
                    <h4 className="font-extrabold text-sm text-stone-900">{getTranslation("noPastRecords", lang)}</h4>
                  ...
                  <div className="flex items-center justify-center gap-3 pt-2">
                    <button
                      onClick={handleStartIntakeFromPortal}
                      className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow-sm flex items-center gap-1.5"
                    >
                      <PlusCircle className="w-4 h-4" /> {getTranslation("startInterview", lang)}
                    </button>
                    <button
                      onClick={() => setActiveTab('documents')}
                      className="px-4 py-2.5 bg-white border border-stone-300 hover:border-emerald-600 text-stone-800 font-bold text-xs rounded-xl flex items-center gap-1.5"
                    >
                      <UploadCloud className="w-4 h-4 text-emerald-600" /> {getTranslation("uploadDocsTitle", lang)}
                    </button>
                  </div>
                </div>
              ) : (
                <div className="space-y-3 text-xs">
                  {visitHistory.map(v => (
                    <div key={v.id} className="p-4 bg-stone-50 border border-stone-200 rounded-xl space-y-2">
                      <div className="flex items-center justify-between flex-wrap gap-2">
                        <div className="font-extrabold text-stone-900 text-sm">{v.hospital}</div>
                        <span className="px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-semibold text-[10px]">
                          {v.date}
                        </span>
                      </div>
                      <div className="text-stone-600"><strong>Chief Complaint:</strong> {v.chiefComplaint}</div>
                      <div className="text-stone-600"><strong>Diagnosis:</strong> {v.diagnosis}</div>
                      <div className="pt-2 flex justify-end">
                        <button
                          onClick={() => setSelectedVisitModal(v)}
                          className="px-3 py-1.5 bg-white border border-stone-300 hover:border-emerald-600 text-stone-800 rounded-lg font-semibold flex items-center gap-1.5"
                        >
                          <Eye className="w-3.5 h-3.5 text-emerald-600" /> View Full OPD Case Summary
                        </button>
                      </div>
                    </div>
                  ))}
        ...
        {activeTab === 'intake' && (
          <div className="bg-white border border-stone-200 rounded-3xl p-4 md:p-6 shadow-xl space-y-6">
            <div className="flex items-center justify-between border-b border-stone-200 pb-4">
              <div>
                ...
                <h2 className="text-xl font-extrabold text-stone-900 mt-1 flex items-center gap-2">
                  <Mic className="w-5 h-5 text-emerald-600" /> Kiosk Voice & Touch Interview
                </h2>
              </div>
              <button
                onClick={() => setActiveTab('overview')}
                className="px-3.5 py-1.5 bg-stone-100 hover:bg-stone-200 text-stone-700 font-bold text-xs rounded-xl border border-stone-300"
              >
        ...
        {activeTab === 'visits' && (
          <div className="space-y-5">
            <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-xs flex items-center justify-between flex-wrap gap-4">
              <div>
                <h2 className="text-lg font-bold text-stone-900 mb-1">{getTranslation("myVisitsReports", lang)}</h2>
                <p className="text-xs text-stone-500">Every visit generated via MediKiosk is securely archived in your ABHA health record.</p>
              </div>
            </div>
            ...
            {visitHistory.length === 0 ? (
              <div className="bg-white border border-stone-200 rounded-2xl p-8 text-center space-y-3">
                ...
                <h3 className="font-bold text-stone-900 text-base">{getTranslation("noPastRecords", lang)}</h3>
                <p className="text-xs text-stone-500 max-w-sm mx-auto">
                  {getTranslation("freshAccountMsg", lang)}
                </p>
              </div>
              ...
              <div className="space-y-4">
                {visitHistory.map(v => (
                  <div key={v.id} className="bg-white border border-stone-200 rounded-2xl p-6 shadow-xs space-y-3 text-xs">
                    <div className="flex items-center justify-between border-b border-stone-100 pb-3 flex-wrap gap-2">
                      <div>
                        <div className="text-sm font-extrabold text-stone-900">{v.hospital}</div>
                        <div className="text-stone-500">{v.department} • Attending: {v.doctor}</div>
                      </div>
                      <span className="px-3 py-1 bg-stone-100 border border-stone-200 font-mono text-stone-700 font-bold rounded-lg">
                        {v.date}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-stone-50 p-4 rounded-xl border border-stone-200">
                      <div>
                        <span className="font-bold text-stone-700 block mb-1">Elicited Chief History:</span>
                        <p className="text-stone-600">{v.chiefComplaint}</p>
                      </div>
                      <div>
                        <span className="font-bold text-stone-700 block mb-1">Clinical Assessment:</span>
                        <p className="text-stone-800 font-semibold">{v.diagnosis}</p>
                      </div>
                    </div>

                    <div className="flex items-center justify-between pt-2">
                      <div className="text-stone-500 text-[11px]">Lab Flags: {v.labSummary}</div>
                      <button
                        onClick={() => setSelectedVisitModal(v)}
        ...
        {activeTab === 'documents' && (
          <div className="space-y-5">
            <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-xs flex items-center justify-between flex-wrap gap-4">
              <div>
                <h2 className="text-lg font-bold text-stone-900 mb-1">{getTranslation("medicalRecordsUploads", lang)}</h2>
                <p className="text-xs text-stone-500">{getTranslation("uploadDocsSub", lang)}</p>
              </div>
              <div className="w-32 h-20 rounded-xl overflow-hidden border border-stone-200 hidden sm:block">
                <img src={MEDICAL_IMAGES.labReport} alt="Lab Scan" className="w-full h-full object-cover" />
              </div>
            </div>
        ...
        {activeTab === 'abha' && (
          <div className="space-y-6">
            <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-xs flex items-center justify-between flex-wrap gap-4">
              <div>
                <h2 className="text-lg font-bold text-stone-900 mb-1">{getTranslation("abhaHealthId", lang)}</h2>
                <p className="text-xs text-stone-500">Official ABDM Digital Health ID card for seamless interoperability across Indian healthcare facilities.</p>
              </div>
            </div>

            {/* ABHA CARD VISUAL REPLICA */}
            <div className="max-w-md mx-auto bg-gradient-to-br from-emerald-800 via-teal-900 to-stone-900 text-white rounded-3xl p-6 shadow-2xl space-y-6 border border-emerald-500/30 relative overflow-hidden">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-lg bg-emerald-400 text-stone-950 font-black flex items-center justify-center text-sm">
                    AB
                  </div>
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider text-emerald-300">National Health Authority</div>
                    <div className="text-[10px] text-stone-300">Ministry of Health & Family Welfare</div>
                  </div>
                </div>
              ...
              <div className="flex items-start justify-between gap-4 pt-2">
                <div className="space-y-1">
                  <div className="text-xs text-emerald-300 font-semibold uppercase">ABHA Address</div>
              ...
              <div className="bg-stone-950/60 p-3 rounded-2xl border border-stone-700/50 flex items-center justify-between font-mono text-sm">
      ...
      {langModalOpen && (
        <div className="fixed inset-0 bg-stone-900/70 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-stone-200 rounded-3xl max-w-md w-full p-6 space-y-5 shadow-2xl">
            <div className="flex items-center justify-between border-b border-stone-100 pb-3">
              <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="text-stone-400 hover:text-stone-700 text-lg font-bold px-2"
              >
                ✕
              </button>
            </div>
            ...
            <div className="space-y-2.5">
              {[
                ...
                <button
                  ...
                  className={`w-full p-3.5 rounded-xl border-2 flex items-center justify-between text-left transition ${
                    lang === l.code
                      ? 'border-emerald-600 bg-emerald-50 text-emerald-950 font-bold shadow-xs'
                      : 'border-stone-200 hover:border-emerald-500 bg-stone-50'
                  }`}
                >
                  <div>
                    <div className="text-sm font-extrabold">{l.label}</div>
                    <div className="text-[11px] text-stone-500 font-normal">{l.desc}</div>
                  </div>
                  {lang === l.code && <Check className="w-5 h-5 text-emerald-600" />}
                </button>
              ))}
            </div>
          </div>
      ...
      {selectedVisitModal && (
        <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-stone-200 rounded-3xl max-w-2xl w-full p-6 space-y-4 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-stone-100 pb-3">
              <h3 className="font-bold text-base text-stone-900 flex items-center gap-2">
                <FileText className="w-5 h-5 text-emerald-600" /> OPD Visit Case Record Summary
              </h3>
              <button
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                <strong className="text-stone-800 block mb-1">Chief Presenting Complaints:</strong>
                <p className="text-stone-600 bg-stone-50 p-3 rounded-lg border border-stone-200">{selectedVisitModal.chiefComplaint}</p>
              </div>

              <div>
                <strong className="text-stone-800 block mb-1">Prescriptions Issued:</strong>
              ...
              <div>
                <strong className="text-stone-800 block mb-1">Lab Findings:</strong>
                <p className="text-stone-600">{selectedVisitModal.labSummary || "Normal"}</p>
              </div>
            </div>
            ...
            <div className="pt-4 flex justify-end">
              <button
                onClick={() => setSelectedVisitModal(null)}
                className="px-4 py-2 bg-stone-800 text-white font-bold text-xs rounded-xl"
              >
                Close Summary
              </button>
            </div>
