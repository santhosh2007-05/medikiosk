...
export const CareTrackLoginPage = ({ onNavigateRegister, onLoginSuccess }) => {
  ...
  const [role, setRole] = useState("doctor"); // 'doctor' | 'nurse' | 'patient' | 'receptionist'
  ...
  const getRoleAvatar = () => {
    switch (role) {
      ...
      default: return ROLE_AVATARS.doctor1;
    }
  };
  ...
  const handleStaffLogin = (e) => {
    e.preventDefault();
    if (role === 'receptionist') setViewMode('receptionist');
          ...
          <div className="p-4 rounded-2xl bg-stone-900/85 backdrop-blur-md border border-stone-700/60 flex items-center gap-3.5 shadow-xl">
            ...
            <div>
              <div className="text-xs font-extrabold text-white uppercase tracking-wider">
                {role === 'doctor' ? 'Doctor OPD Workstation' : role === 'nurse' ? 'Nurse Intervention Desk' : role === 'receptionist' ? 'Reception Counter Desk' : 'Patient Portal'}
              </div>
          ...
          <div className="space-y-6">
            ...
            {role !== 'patient' ? (
              ...
              <form onSubmit={handleStaffLogin} className="space-y-4">
                ...
                <button type="submit" className="w-full p-3 bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold rounded-xl shadow-lg transition flex items-center justify-center gap-2 text-sm mt-2">
                  Sign in to {role.toUpperCase()} Portal <ArrowRight className="w-4 h-4" />
                </button>
              </form>
            ) : (
